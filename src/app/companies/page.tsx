...
export default async function CompaniesPage() {
    ...
    const serializedData = companies.map((company) => ({ ...company }));

    const totalCompanies = companies.length;
    const totalPendingWorkOrders = companies.reduce((sum, company) => sum + parseFloat(company.workOrderTotalPending), 0);
    const totalPendingOrders = companies.reduce((sum, company) => sum + parseFloat(company.orderTotalPending), 0);
