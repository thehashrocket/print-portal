...
const UsersTable: React.FC<User[]> = (users) => {
    ...
    const columnDefs = [
        ...
        { headerName: 'Permissions', field: 'permissions', sortable: true, filter: true },
    ];

    useEffect(() => {
        console.log('users', users['users']);
        setRowData(users['users'].map((user) => {
            return {
                name: user.name,
                email: user.email,
                roles: user.roles.map((role) => role.name).join(', '),
                permissions: user.roles.map((role) => role.permissions.map((permission) => permission.name).join(', ')).join(', '),
            }
        }));
    }, []);
