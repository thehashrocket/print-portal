...
function MyComponent() {
  const { data, isLoading } = api.getByCompanyId.useQuery("example");
...
async function MyServerComponent() {
  const data = await api.getByCompanyId.query("example");
...
function MyComponent() {
  ...
  const mutation = api.deleteAddress.useMutation({
    onSuccess: () => {
      setIsLoading(false);
      setSuccess("Operation completed successfully!");
      setError(null);
...
const mutation = api.deleteAddress.useMutation({
  onMutate: async (newData) => {
    await utils.deleteAddress.cancel();
    const previousData = utils.deleteAddress.getData();

    utils.deleteAddress.setData(undefined, (old) => {
      // Update logic here
      return old;
    });
  ...
  onError: (err, newData, context) => {
    utils.deleteAddress.setData(undefined, context?.previousData);
  },
  onSettled: () => {
    utils.deleteAddress.invalidate();
  },
});
```
