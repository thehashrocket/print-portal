...
function MyComponent() {
  ...
  const mutation = api.createContact.useMutation({
    onSuccess: () => {
      setIsLoading(false);
...
const mutation = api.createContact.useMutation({
  onMutate: async (newData) => {
    await utils.createContact.cancel();
    const previousData = utils.createContact.getData();

    utils.createContact.setData(undefined, (old) => {
      // Update logic here
      return old;
    });
  ...
  onError: (err, newData, context) => {
    utils.createContact.setData(undefined, context?.previousData);
  },
  onSettled: () => {
    utils.createContact.invalidate();
  },
});
```
