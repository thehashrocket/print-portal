...
const randomElementFromArray = <T>(array: T[]): T => array[Math.floor(Math.random() * array.length)];
...
async function createWorkOrder(officeId) {
    ...
    const workOrder = await prismaClient.workOrder.create({
        data: {
            ...
            costPerM: faker.number.int({ min: 50, max: 200 }),
            totalCost: faker.number.int({ min: 500, max: 10000 }),
...
async function createWorkOrderStock(workOrderId) {
    ...
    return await prismaClient.workOrderStock.create({
        data: {
            ...
            notes: faker.lorem.sentence(),
            stockStatus: randomElementFromArray(['InStock', 'OnHand', 'CS', 'Ordered', 'OutOfStock', 'LowStock']),
        },
    });
}
...
async function createRolesAndPermissions() {
    ...
    const permissions = [
        ...
        { name: 'order_payment_change_status', description: 'Change order payment status' },
        ...
        { name: 'order_shipping_info_create', description: 'Create order shipping info' },
        { name: 'order_shipping_info_delete', description: 'Delete order shipping info' },
        { name: 'order_shipping_info_read', description: 'Read order shipping info' },
        { name: 'order_shipping_info_update', description: 'Update order shipping info' },
        ...
        { name: 'typesetting_option_create', description: 'Create typesetting option' },
        { name: 'typesetting_option_delete', description: 'Delete typesetting option' },
        { name: 'typesetting_option_read', description: 'Read typesetting option' },
        { name: 'typesetting_option_update', description: 'Update typesetting option' },
        { name: 'typesetting_proof_create', description: 'Create typesetting proof' },
        { name: 'typesetting_proof_read', description: 'Read typesetting proof' },
        { name: 'typesetting_proof_update', description: 'Update typesetting proof' },
        ...
        { name: 'work_order_change_status', description: 'Change work order status' },
...
async function convertWorkOrderToOrder(workOrderId, officeId) {
    const order = await prismaClient.order.create({
        data: {
            ...
            plateRan: faker.word.sample(),
            expectedDate: faker.date.future(),
            deposit: faker.number.int({ min: 100, max: 5000 }),
            costPerM: faker.number.int({ min: 50, max: 200 }),
            totalCost: faker.number.int({ min: 500, max: 10000 }),
            binderyTime: faker.word.sample(),
            overUnder: faker.word.sample(),
